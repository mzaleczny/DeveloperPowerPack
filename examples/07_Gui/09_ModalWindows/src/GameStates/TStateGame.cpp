...
TStateGame::TStateGame(Tilc::TStateManager* StateManager) : Tilc::TBaseState(StateManager, true)
{
    ...
    Tilc::Gui::TStyledWindow* ModalWnd = new Tilc::Gui::TStyledWindow(
        wnd,
        "ModalWindow1",
        { 300.0f, 200.0f, Tilc::GameObject->m_Window->GetWindowWidth() / 3.0f, Tilc::GameObject->m_Window->GetWindowHeight() / 2.0f }
    );
    ModalWnd->SetText("Okno modalne #1");
    ModalWnd->SetModal();
}
