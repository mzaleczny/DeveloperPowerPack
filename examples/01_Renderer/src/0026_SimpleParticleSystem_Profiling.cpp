...
Tilc::TProfiler Profiler(1);
constexpr int CounterThreshold = 100;
int Counter = 0;
...
SDL_AppResult SDL_AppInit(void** appstate, int argc, char* argv[])
{
    ...
    Profiler.Reset(CounterThreshold);
...
SDL_AppResult SDL_AppIterate(void* appstate)
{
    ...
    {
        Profiler.Begin("FramebufferDraw");
        ++Counter;
    ...
    else if (Counter < (CounterThreshold << 1))
    {
        Profiler.Begin("SurfaceDraw");
        ++Counter;
        SDL_Surface* surface;
        if (SDL_LockTextureToSurface(texture, NULL, &surface))
        {
            ...
            for (std::size_t i = 0; i < Particles.size(); ++i)
            {
                ...
                {
                    c = (int)Particles[i].position.y * surface->pitch + (int)Particles[i].position.x * 4;
                    char* Pixel = &((char*)surface->pixels)[c];
                    char Value = (char)(Particles[i].color.a * 255.0f);
                    Pixel[0] = Value;   // R
                    Pixel[1] = Value;   // G
                    Pixel[2] = Value;   // B
...
{
    Tilc::TExtString ProfilerBuffer;
    Profiler.DumpToBuffer(ProfilerBuffer);
    SDL_Log("%s", ProfilerBuffer.c_str());
    if (FrameBuffer)
    {
        delete[] FrameBuffer;
    }
