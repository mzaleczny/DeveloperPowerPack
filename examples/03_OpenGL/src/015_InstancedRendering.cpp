...
{
	std::string WindowTitle("Instanced Rendering");
	SDL_SetAppMetadata(WindowTitle.c_str(), "1.0", "pl.net.progmar.instanced-rendering");
...
SDL_AppResult SDL_AppIterate(void* appstate)
{
	...
	{
		ProjectionMatrix * ViewMatrix * Tilc::Translate(ModelMatrix, {100.0f, 0.0f, 0.0f}),
		ProjectionMatrix * ViewMatrix * Tilc::Translate(ModelMatrix, {0.0f, 0.0f, -100.0f}),
		ProjectionMatrix * ViewMatrix * Tilc::Translate(ModelMatrix, {100.0f, 0.0f, -100.0f})
	};
	...
	for (uint32_t i = 0; i < Meshes.size(); ++i)
	{
		if (Meshes[i])
		{
			...
			Meshes[i]->m_Pipeline->SetMat4Array(Meshes[i]->m_Pipeline->m_VertexShader, "Model", MVPMatrix[0].Elems, 3);
