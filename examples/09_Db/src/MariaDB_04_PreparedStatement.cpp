...
int main(int argc, char* argv[])
{
    ...
    if (Conn)
    {
        try
        {
            ...
            stmt->executeQuery("UPDATE products set price=price/2");
            pstmt.reset(Conn->prepareStatement("UPDATE products SET price=? WHERE id=?"));
            // Warning! Decimals better to set as strings not as floats as call below sets 1000000.00 insted of 999999.99
            //pstmt->setFloat(1, 999999.99);
            // Line below works fine
            pstmt->setString(1, "999999.99");
            pstmt->setFloat(2, 5);
            pstmt->executeUpdate();

            pstmt.reset(Conn->prepareStatement("DELETE FROM products WHERE id=?"));
            pstmt->setInt(1, 6);
            pstmt->executeUpdate();
