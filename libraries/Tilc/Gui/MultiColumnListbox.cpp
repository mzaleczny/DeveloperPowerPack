...
void Tilc::Gui::TMultiColumnListbox::Draw(SDL_Texture* Canvas, SDL_FRect* Position)
{
    ...
    if (m_ColumnWidths.size() > 0)
    {
        for (size_t i = 0; i < m_ColumnWidths.size(); ++i)
        {
            ...
            HeaderOffsetX += m_ColumnWidths[i];
        }
    }
    // ================================================================
    // ================================================================
...
bool Tilc::Gui::TMultiColumnListbox::OnMouseButtonDown(const SDL_Event& event)
{
    ...
    if (PointIn(event.button.x, event.button.y))
    {
        ...
        if (!m_Header.m_Value.empty())
        {
            HeaderHeight = t->listbox_header_middle_rc.h;
        }
        int ClickedItem = (event.button.y - HeaderHeight - m_RealPosition.y - t->listbox_frame_top_rc.h - m_Padding);
        if (ClickedItem >= 0)
        {
            if (m_VScrollBar)
            {
