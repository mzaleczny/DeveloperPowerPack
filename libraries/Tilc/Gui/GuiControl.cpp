...
void Tilc::Gui::TGuiControl::AddHorizontalScrollbar(int min, int max, int size, bool IsStandardSizedScrollBar)
{
    if (Tilc::GameObject)
    {
        ...
        if (scrbar)
        {
            if (!size)
            {
                size = RealPosition.w - vscr_w;
            }
            scrbar->SetSizeRelativeToParent(size, RealPosition.h);
            scrbar->RegisterPositionNotificationForControl(this);
            m_HScrollBar = scrbar;
        }
    }
...
void Tilc::Gui::TGuiControl::DrawCommon(const SDL_FRect& Position, const SDL_FRect& rc, const SDL_FRect& disabled_rc, const SDL_FRect& focused_rc, const SDL_FRect& hover_focused_rc, const SDL_FRect& pushed_focused_rc, const SDL_FRect& hover_rc, const SDL_FRect& pushed_rc)
{
    ...
    RenderTexture(TextureMap, &ctrl, Position.x, Position.y);
}
